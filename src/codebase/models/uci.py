...
import pandas as pd
import os
import numpy as np  # Keep regular numpy for data loading operations


def load_dataset(name, split_seed=0, test_fraction=0.1):
    """
    Load a UCI dataset and split into train and test sets
    ...
    # Use JAX's PRNG system
    key = jax.random.PRNGKey(split_seed)

    # Generate permutation using JAX
    permutation = jax.random.permutation(key, X.shape[0])
    size_train = int(jnp.round(X.shape[0] * (1 - test_fraction)))

    # Use JAX indexing for train/test split
